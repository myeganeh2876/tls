...
class TalkingHead {
   ...
   */
  lipsyncGetProcessor(lang, path="../modules/") {
      ...
      if ( item.anim ) {
        item.anim.forEach( x => {
          for(let i=0; i<x.ts.length; i++) {
            ...
            totalDuration += this.animClock + x.ts[i] + delay;
          }
          this.animQueue.push(x);
          setTimeout( this.playAudio.bind(this), totalDuration / 10, true );
        ...
        try {
          ...
          line.text.forEach( (x,i) => {
            ...
            if (i > 0) {
              ...
              timepoints.push( { mark: i, time: timepoints[i - 1].time + timepoints[i - 1].duration, duration: x.word.length * 70 });
            }else{
              timepoints.push( { mark: i, time: 0, duration: x.word.length * 70 });
            }
...
//           const audio = await this.audioCtx.decodeAudioData( buf );
          ...
          line.anim.forEach( x => {
            ...
            // Add to the playlist
            this.audioPlaylist.push({ anim: line.anim, audio: null });
